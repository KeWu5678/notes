...
\begin{enumerate}
	...
	\item $\mathbb{P}^X(\cdot, B)$ is a version of conditional expectation of $\mathbb{E}[\mathbbm{1}_{B}|\mathfrak{G}]$ for any $B \in \mathfrak{E }$.
\end{enumerate}
\end{definition}
...
\end{remark}
\begin{proof}
	\textbf{1)} Show $A_n$ is a predictable process. (Also $A_{n+1}$ a submartingale?) It's trivial to see that
	\[
		\mathbb{E}[A_n|\mathcal{F}_{n-1}] = \mathbb{E}[X_n - X_{n-1}|\mathcal{F}_{n-1}] + A_{n-1}.
	\]
	Notice the conditional expectation on the right-hand side is non-negative, therefore $A_{n-1} \leq  \mathbb{E}[A_n|\mathcal{F}_{n-1}]$ a.s.

	\textbf{2)} Show that $M_n:= X_n - X_0 - A_n$ is a martingale.
...
$(X_n)$ be i.i.d  random variable with $\mathbb{E}X_n = 0$ , $\var X_n < \infty$. Notice $S_n$ is a martingale with its natural filtration. Observe the tail $\sigma$-field:
\begin{align*}
...
\begin{align*}
	\mathbb{P}( \lim_{n \rightarrow \infty}( S_n > \sqrt{n} \cdot a))   = 1 - \Phi(\frac{a}{\sqrt{\var X_1}})
\end{align*}
...
\begin{align*}
	\forall A \in \mathcal{F}_\sigma: \mathbb{E}[X_\tau \mathbbm{1}_A] = \mathbb{E}[X_\sigma \mathbbm{1}_A]
\end{align*}
Observe:
\begin{align*}
...
\begin{theorem}[The First Martingale convergence theorem]
	Let $X$ be a submartingale with $\sup_n \mathbb{E} X_n^+ < \infty$. Then
\begin{align*}
		\lim_{n \rightarrow \infty} X_n = X_\infty \quad \text{exists a.s.} \quad \text{and} \quad X_\infty \in L^1.
\end{align*}
\end{theorem}
\begin{proof}
	For $a < b$ let $U_n(a, b)$ denote the number of upcrossings of the interval $[a, b]$ by $X_0, \dots, X_n$ (as in Doob's upcrossing theorem), and set
	\begin{align*}
		U(a, b) := \lim_{n \rightarrow \infty} U_n(a, b) = \sup_n U_n(a, b),
	\end{align*}
	the total number of upcrossings; the limit exists since $n \mapsto U_n(a, b)$ is non-decreasing. Write $C := \sup_n \mathbb{E} X_n^+ < \infty$, and recall the decomposition $X = X^+ - X^-$ with $X^+ = \max(X, 0)$, $X^- = \max(-X, 0)$.\\
	\textbf{Step 1: $U(a, b) < \infty$ a.s.} By Doob's upcrossing theorem together with the bound $(X_n - a)^+ \le X_n^+ + |a|$,
	\begin{align*}
		\mathbb{E} U_n(a, b) \le \frac{1}{b - a} \mathbb{E}(X_n - a)^+ \le \frac{1}{b - a}\big( C + |a| \big),
	\end{align*}
	uniformly in $n$. Since $U_n(a, b) \uparrow U(a, b)$, monotone convergence gives
	\begin{align*}
		\mathbb{E} U(a, b) = \lim_{n \rightarrow \infty} \mathbb{E} U_n(a, b) \le \frac{1}{b - a}\big( C + |a| \big) < \infty,
	\end{align*}
	hence $U(a, b) < \infty$ a.s.\\
	\textbf{Step 2: almost sure convergence.} The limit $\lim_n X_n$ fails to exist in $[-\infty, +\infty]$ precisely when $\liminf_n X_n < \limsup_n X_n$, that is on the event
	\begin{align*}
		\Big\{ \liminf_n X_n < \limsup_n X_n \Big\} = \bigcup_{\substack{a < b,\ a, b \in \mathbb{Q}}} \big\{ \liminf_n X_n < a < b < \limsup_n X_n \big\}.
	\end{align*}
	On each set $\{ \liminf_n X_n < a < b < \limsup_n X_n \}$ the process upcrosses $[a, b]$ infinitely often, so it is contained in $\{ U(a, b) = \infty \}$, which is null by Step 1. As a countable union of null sets, the left-hand event is null. Therefore $\liminf_n X_n = \limsup_n X_n =: X_\infty$ a.s., i.e.\ $X_n \rightarrow X_\infty$ a.s.\ (with values in $[-\infty, +\infty]$).\\
	\textbf{Step 3: $X_\infty \in L^1$.} Since $X_n \rightarrow X_\infty$ a.s., Fatou's lemma yields
	\begin{align*}
		\mathbb{E} X_\infty^+ \le \liminf_n \mathbb{E} X_n^+ \le C < \infty.
	\end{align*}
	For the negative part, the submartingale property gives $\mathbb{E} X_n \ge \mathbb{E} X_0$, so
	\begin{align*}
		\mathbb{E} X_n^- = \mathbb{E} X_n^+ - \mathbb{E} X_n \le \mathbb{E} X_n^+ - \mathbb{E} X_0,
	\end{align*}
	and Fatou's lemma gives
	\begin{align*}
		\mathbb{E} X_\infty^- \le \liminf_n \mathbb{E} X_n^- \le C - \mathbb{E} X_0 < \infty.
	\end{align*}
	Hence $\mathbb{E}|X_\infty| = \mathbb{E} X_\infty^+ + \mathbb{E} X_\infty^- < \infty$, so $X_\infty \in L^1$; in particular $X_\infty$ is finite a.s.
\end{proof}
\begin{remark}
...
\begin{proof}
	...
	therefore:
	\begin{align*}
		M_{-n} = \frac{1}{n}\mathbb{E}[S_n|\mathcal{F}_{-n}] = \frac{1}{n}S_n
	\end{align*}
...
\begin{corollary}
 ...
 \begin{lemma}
 	...
 	\begin{align*}
 		&\mathbb{P}(X_0 = i_0, \dots, X_{m+n} = i_{m+n}, \dots|X_m = i)\\
		&\quad = \mathbb{P}(X_0 = i_0, \dots,X_m = i_i |X_m = i)
		\delta_{i,i_m} p_{i_n, i_{n+1}}\dots, p_{i_{m+n-1}, i_{m+n}}\dots
 	\end{align*}
 \end{lemma}
  \begin{proof}
...
\begin{align*}
	...
	&\quad + \mathbb{P}(X_1 \notin A, X_{n-1}\notin A, X_n \in A)\\
	&\quad + \sum_{p_{ij_1}, p_{ij_2}, \dots, p_{ij_n} \notin A}
	p_{ij_1} \dots p_{j_{n-1}j_n}\hat{h}_n^A\\
...
\begin{proof}\quad
 	\begin{enumerate}
	 		\item First let be the simple function $Y: \textbf{1}(X_0 = i_0, \dots, X_n = i_n)$ and
			\[
				A:= \{(X_0, \dots, X_n) \in B_n \mid B_n \subset S^{n+1}\} \subset \mathcal{F}_n .
			\]
	 		\begin{align*}
	 			\mathbb{E}_\mu[Y\circ \theta^n \textbf{1}_A]
			&= \sum_{b_0, \dots, b_n \in B_n}\\
			&\quad \mathbb{E}_\mu[\textbf{1}(X_n = i_0, \dots, X_{n+m} = i_m)
			\textbf{1}(X_0 = b_0, \dots, X_n = b_n)]\\
 			&= \sum_{b_0, \dots, b_n \in B_n}\\
			&\quad \mathbb{P}_\mu(X_0= b_0, \dots, X_n = b_n)
			\mathbb{P}_\mu(X_n = i_0, \dots, X_{n+m} = i_m|X_n = b_n)\\
 			(time\text{-}homogeneous)&= \sum_{b_0, \dots, b_n \in B_n}\\
			&\quad \mathbb{P}_\mu(X_0= b_0, \dots, X_n = b_n)
			\mathbb{P}_{b_n}(X_0 = i_0, \dots, X_{m} = i_m)\\
...
\end{theorem}
	...
	\begin{align*}
		...
		&\quad = \mathbb{P}(\{ X_\tau = j_0 \dots \}|X_\tau = i, \tau < \infty)
		\mathbb{P}(A|X_\tau = i, \tau < \infty)\\
	...
	\begin{align*}
		...
		&\overset{\tau < \infty} = \sum_{m \ge 0}\\
		&\quad \mathbb{P}(\{X_m = j_0, X_{m + 1} = j_1, X_{m + n} = j_n\} \cap A \cap  \{\tau = m\}| X_\tau = i, \tau < \infty )\\
		(Bayes) &= \sum_{m \geq 0}
		\mathbb{P}(\{ X_m = j_0, X_{\tau + 1} = j_1, \dots, X_{m + n} = j_n\}\\
		&\quad \cap A \cap\{\tau = m\} \cap  \{X_m = i\} )
		\frac{1}{\mathbb{P}(X_\tau = i, \tau < \infty)} \\
		(past\perp future) 
		&= \sum_{m \geq 0}
		\mathbb{P}(\{X_m = j_0,\dots, X_{m + n} = j_n\}| X_m = i)\\
		&\quad
		\mathbb{P}(A \cap \{\tau = m\}|X_m = i)
		\frac{\mathbb{P}(X_m = i)}{P(X_\tau = i, \tau < \infty)} \\
		(Homogeneous)&= \mathbb{P}(X_0 = j_0, X_1 = j_1, \dots X_n = j_n|X_0 = i)\\
		&\quad
		\sum_{ m \geq 0}\frac{\mathbb{P}(A \cap \{\tau = m\})}{\mathbb{P}(\tau < \infty, X_\tau = i)}\\
...
\begin{proof}
	...
	\begin{align*}
		\tilde{\lambda} = \lambda - \lambda^k \text{ is invariant}, \quad \tilde{\lambda}_k = 0
	\end{align*}
	since $X$ irreducible
	\begin{align*}
		p_{jk}^n  >0
...
\begin{definition}[Weak convergence of probability measures]
	A sequence of probability measures $(\mathbb{P}_n)_{n \in \mathbb{N}}$ on a metric space $(E, \mathcal{E})$ \textbf{converges weakly} to some probability measure $\mathbb{P}$ on $(E, \mathcal{E})$, written $\mathbb{P}_n \Rightarrow \mathbb{P}$, if
	\begin{align*}
		\int_E g(x)\, \mathbb{P}_n(dx) \rightarrow \int_E g(x)\, \mathbb{P}(dx)
	\end{align*}
	for all bounded and continuous functions $g: E \rightarrow \mathbb{R}$.
\end{definition} 
...
Let $(\mathbb{P}_i)_{i \in \mathbb{N}}$ and $\mathbb{P}$ be probability measures on a metric space. The following statements are equivalent:
\begin{enumerate}
	\item $\mathbb{P}_i$ converges weakly to $\mathbb{P}$.
	\item For every closed set $F$,
	\[
		\limsup_{i \to \infty} \mathbb{P}_i(F) \leq \mathbb{P}(F).
	\]
	\item For every open set $G$,
	\[
		\liminf_{i \to \infty} \mathbb{P}_i(G) \geq \mathbb{P}(G).
	\]
	\item For every Borel set $A$ such that $\mathbb{P}(\partial A) = 0$,
	\[
		\mathbb{P}_i(A) \rightarrow \mathbb{P}(A).
	\]
\end{enumerate}
\end{theorem}
...
\begin{lemma}
Let $\mathcal{P}$ be a family of probability measures on a Polish space $(E,\mathcal{B}(E))$.
If $\mathcal{P}$ is weakly relatively sequentially compact, then $\mathcal{P}$ is tight.
\end{lemma}
\begin{proof}
Let $(x_i)_{i\in\mathbb{N}} \subset E$ be dense, and for $n,N\in\mathbb{N}$ define
\[
	A_{n,N}:=\bigcup_{i=1}^N B_{1/n}(x_i).
\]
Then $A_{n,N}\uparrow E$ as $N\to\infty$ for any fixed $n\in\mathbb{N}$. We claim that
\[
	\delta:=\sup_{n\in\mathbb{N}}\inf_{N\in\mathbb{N}}
	\sup_{\mathbb{Q}\in\mathcal{P}}\mathbb{Q}(A_{n,N}^c)=0.
\]
Suppose not. Then there exists $n\in\mathbb{N}$ such that for every $N\in\mathbb{N}$
there exists $\mathbb{P}_N\in\mathcal{P}$ with
\[
	\mathbb{P}_N(A_{n,N}^c)\ge \delta/2.
\]
Since $\mathcal{P}$ is weakly relatively sequentially compact, $(\mathbb{P}_N)$ has a weakly convergent
subsequence $(\mathbb{P}_{N_k})$ with $\mathbb{P}_{N_k}\Rightarrow\mathbb{P}$.
For fixed $N$, the set $A_{n,N}^c$ is closed, and for $k$ large enough we have
$A_{n,N_k}^c\subset A_{n,N}^c$. By the Portmanteau theorem,
\[
	\mathbb{P}(A_{n,N}^c)\ge \limsup_{k\to\infty}\mathbb{P}_{N_k}(A_{n,N}^c)
	\ge \limsup_{k\to\infty}\mathbb{P}_{N_k}(A_{n,N_k}^c)\ge \delta/2.
\]
At the same time $A_{n,N}^c\downarrow\varnothing$, hence $\mathbb{P}(A_{n,N}^c)\to 0$, a contradiction.
Therefore $\delta=0$.

Let $\varepsilon>0$. For every $n\in\mathbb{N}$ choose $N_n\in\mathbb{N}$ such that
\[
	\sup_{\mathbb{Q}\in\mathcal{P}}\mathbb{Q}(A_{n,N_n}^c)<\varepsilon 2^{-n}.
\]
Set
\[
	A:=\bigcap_{n=1}^\infty A_{n,N_n}.
\]
The set $A$ is totally bounded, hence its closure $K:=\overline{A}$ is compact because $E$ is
Polish. For every $\mathbb{Q}\in\mathcal{P}$,
\[
	\mathbb{Q}(K^c)\le \mathbb{Q}(A^c)\le \sum_{n=1}^\infty \mathbb{Q}(A_{n,N_n}^c)
	\le \sum_{n=1}^\infty \varepsilon 2^{-n}=\varepsilon.
\]
Thus $\mathcal{P}$ is tight.
\end{proof}
...
\subsection{\texorpdfstring{Weak convergence on $C([0, T])$, $C(\mathbb{R})$}{Weak convergence on C([0, T]), C(R)}}
...
\begin{definition}[The pre-brownian motion]
	...
	\end{itemize}
	The pre-brownian motion is called a \textbf{brownian motion} if all its trajectories are continuous a.s..
\end{definition}
\subsection{CoInstruction and Path Properties}
...
\begin{proof}
	\quad\\
	\textbf{Measurability}: Define for each $t$ the restriction $X^t: (\Omega \times [0,t], \mathcal{F}_t \otimes \mathcal{B}([0,t])) \to (\mathbb{R}, \mathcal{B})$ by $X^t(\omega, s) := X_s(\omega)$, which is measurable by progressive measurability. 
	Extend to $\bar{X}^t(\omega, s) := X_{s \wedge t}(\omega)$ on all of $\Omega \times [0,\infty)$. 
	This is $\mathcal{F}_t \otimes \mathcal{B}([0,\infty))$-measurable (composition of $X^t$ with the measurable map $(s, \omega) \mapsto (s \wedge t, \omega)$), hence $\mathcal{F} \otimes \mathcal{B}$-measurable since $\mathcal{F}_t \subset \mathcal{F}$. 
	Then $X_s(\omega) = \lim_{t \to \infty} \bar{X}^t(\omega, s)$ is $\mathcal{F} \otimes \mathcal{B}$-measurable as a pointwise limit.\\
...
\section{\texorpdfstring{$\sigma$}{sigma}-algebra and Stopping Time}
...
\end{proposition}
\begin{proof}
	We first show that the stopped process $X^\tau = (X_{\tau \wedge t})_{t \ge 0}$ is progressively measurable, and then deduce the $\mathcal{F}_\tau$-measurability of $X_\tau$.\\
	\textbf{Step 1: $X^\tau$ is progressively measurable.} Fix $t \ge 0$ and consider the two maps
	\begin{align*}
		\Phi: ([0,t] \times \Omega, \mathcal{B}_t \otimes \mathcal{F}_t) &\to ([0,t] \times \Omega, \mathcal{B}_t \otimes \mathcal{F}_t), \quad (s, \omega) \mapsto (\tau(\omega) \wedge s,\, \omega),\\
		\Psi: ([0,t] \times \Omega, \mathcal{B}_t \otimes \mathcal{F}_t) &\to (\mathbb{R}, \mathcal{B}), \quad (s, \omega) \mapsto X_s(\omega).
	\end{align*}
	The map $\Psi$ is measurable by progressive measurability of $X$. For $\Phi$: since $\tau$ is a stopping time, $\omega \mapsto \tau(\omega) \wedge t$ is $\mathcal{F}_t$-measurable, hence $(s, \omega) \mapsto \tau(\omega) \wedge s = (\tau(\omega) \wedge t) \wedge s$ is the minimum of two $\mathcal{B}_t \otimes \mathcal{F}_t$-measurable functions, with values in $[0, t]$; thus $\Phi$ is $(\mathcal{B}_t \otimes \mathcal{F}_t)$-measurable. Therefore the composition
	\begin{align*}
		\Psi \circ \Phi : (s, \omega) \mapsto X_{\tau(\omega) \wedge s}(\omega)
	\end{align*}
	is $\mathcal{B}_t \otimes \mathcal{F}_t$-measurable, i.e.\ $X^\tau$ is progressively measurable.\\
	\textbf{Step 2: $X_\tau$ is $\mathcal{F}_\tau$-measurable.} In particular $X^\tau$ is adapted, so $X_t^\tau = X_{\tau \wedge t}$ is $\mathcal{F}_t$-measurable. On $\{\tau \le t\}$ we have $X_\tau = X_{\tau \wedge t} = X_t^\tau$, hence for every Borel set $B$,
	\begin{align*}
		\{X_\tau \in B\} \cap \{\tau \le t\} = \{X_t^\tau \in B\} \cap \{\tau \le t\} \in \mathcal{F}_t,
	\end{align*}
	since $\{X_t^\tau \in B\} \in \mathcal{F}_t$ and $\{\tau \le t\} \in \mathcal{F}_t$. As this holds for all $t \ge 0$, $X_\tau$ is $\mathcal{F}_\tau$-measurable.
\end{proof}
...
\begin{theorem}[Doob's Optional Sampling Theorem]
	Let $(M_t)$ be a uniformly integrable right-continuous martingale (e.g.\ $M$ is $L^p$-bounded for $p > 1$). Let $\tau, \sigma$ be finite stopping times. Then
	\begin{align*}
		\mathbb{E}[M_\tau \mid \mathcal{F}_\sigma] = M_{\tau \wedge \sigma} \quad \text{and} \quad \mathbb{E}[M_\tau \mid \mathcal{F}_{\sigma+}] = M_{\tau \wedge \sigma} \quad \text{a.s.}
	\end{align*}
	In particular, the stopped process $M^\tau = (M_{t \wedge \tau})$ is a martingale. Similar results hold for sub-martingales.
\end{theorem}
\begin{proof}
...
\section{Quadratic variation and Ito's formula}
\subsection{Quadratic Variation}

Let us fix a \emph{continuous} function $X : [0,T] \rightarrow \mathbb{R}^n$. We denote the components of $X$ by $X^i$ ($i = 1, \dots, n$).

\begin{definition}[Partition]
	A partition $\Pi$ of the time interval $[0,T]$ is a set of points $0 = t_0 < t_1 < \cdots < t_n = T$. The mesh is
	\begin{align*}
		|\Pi| := \max_i |t_i - t_{i-1}|.
	\end{align*}
\end{definition}

\begin{definition}[totol variation and quadratic variation]
	Given a function function $X^i : [0,T] \rightarrow \mathbb{R}$, 
	\begin{itemize}
		\item The \textbf{(total) variation} along the partition $\Pi = \{t_0, \dots, t_n\}$ of $[0,T]$ is defined by
		\begin{align*}
			V_T(X^i, \Pi) := \sum_{j=1}^n |X^i(t_j) - X^i(t_{j-1})|.
		\end{align*}
		The (total) variation on $[0,T]$ is defined by
		\begin{align*}
			V_T(X^i) := \sup \left\{ V_T(X^i, \Pi) : \Pi \text{ is a partition of } [0,T] \right\}.
		\end{align*}
		If $V_T(X^i) < \infty$, then $X^i$ is of \textbf{bounded variation} on $[0,T]$.
		\item The co-variation process of the functions $X^j, X^k : [0,T] \rightarrow \mathbb{R}$ along the partition $\Pi = \{t_0, \dots, t_n\}$ of $[0,T]$ is given by
		\begin{align*}
			V_t^2(X^j, X^k, \Pi) = \sum_{t_{i-1}, t_i \in \Pi,\, t_i \le t} \left( X^j(t_i) - X^j(t_{i-1}) \right)\left( X^k(t_i) - X^k(t_{i-1}) \right).
		\end{align*}
		The \textbf{quadratic variation process} of $X^i$ along $\Pi$ is given by
		\begin{align*}
			V_t^2(X^i, \Pi) = V_t^2(X^i, X^i, \Pi), \quad t \in [0,T].
		\end{align*}
		\item We say that $X$ is of \textbf{continuous quadratic variation} on $[0,T]$ if the limit
		\begin{align*}
			\langle X^j, X^k \rangle_t := \lim_{n \rightarrow \infty} V_t^2(X^j, X^k, \Pi^n)
		\end{align*}
		exists for all $j, k = 1, \dots, n$ along a sequence of partitions $\{\Pi^n\}$ with $|\Pi^n| \rightarrow 0$ as $n \rightarrow \infty$ and the map $t \mapsto \langle X, Y \rangle_t$ is continuous. The quadratic variation process $\langle X^i \rangle$ of the function $X^i$ is defined by
		\begin{align*}
			\langle X^i \rangle := \langle X^i, X^i \rangle.
		\end{align*}
	\end{itemize}
\end{definition}

The process of continuous quadratic variation is the central concept. 
It not only requires the limit of quadratic variation exists as the mesh of partition goes to 0. 
It also requires the map from $t$ to $V_t$ is continuous. These are both strong assumptions.

\begin{lemma}[properties]
	Let $M$ be a square integrable martingale and $0 \le s \le t \le u \le v < \infty$. Then
	\begin{enumerate}
		\item increments over disjoint time intervals are orthogonal in $L^2$,
		\begin{align*}
			\mathbb{E}[(M_v - M_u)(M_t - M_s)] = 0;
		\end{align*}
		\item $\mathbb{E}\big[ (M_v - M_u)^2 \,\big|\, \mathcal{F}_t \big] = \mathbb{E}\big[ M_v^2 - M_u^2 \,\big|\, \mathcal{F}_t \big]$;
		\item for any partition $0 = t_0 < t_1 < \dots < t_n = t$ with increments $\Delta_i M := M_{t_i} - M_{t_{i-1}}$,
		\begin{align*}
			\mathbb{E}\Big[ \Big( \sum_i \Delta_i M \Big)^2 \Big] = \sum_i \mathbb{E}\big[ (\Delta_i M)^2 \big].
		\end{align*}
	\end{enumerate}
\end{lemma}


The next theorem shows that $X$ has quadratic variation zero if it is of bounded variation. 
We deduce that all monotone processes are of trivial quadratic variation.
\begin{theorem}
	If $X : [0, T] \rightarrow \mathbb{R}$ is continuous and of bounded variation, then its quadratic variation process is trivial, i.e., identically equal to zero.
\end{theorem}
\begin{proof}
	For any partition $\Pi^n = (t_0^n, \dots, t_{m_n}^n)$ we have that
	\begin{align*}
		\sum_{t_{i-1}^n, t_i^n \in \Pi^n,\, t_i^n \le t} \left( X(t_i^n) - X(t_{i-1}^n) \right)^2
		&\le \sup_i |X(t_i^n) - X(t_{i-1}^n)| \cdot \sum_{t_{i-1}^n, t_i^n \in \Pi^n,\, t_i^n \le t} |X(t_i^n) - X(t_{i-1}^n)|\\
		&\le \sup_i |X(t_i^n) - X(t_{i-1}^n)| \, V_T(X).
	\end{align*}
	Since $X$ is continuous and hence uniformly continuous on compact sets,
	\begin{align*}
		\lim_{n \rightarrow \infty} \sup_i |X(t_i^n) - X(t_{i-1}^n)| = 0 \quad \text{if } |\Pi^n| \rightarrow 0.
	\end{align*}
\end{proof}


\subsection{Quadratic Variation and Brownian Motion}
We are now going to compute the quadratic variation of Brownian motion. It turns out that the quadratic variation can be computed as a \textit{deterministic} limit for almost every Brownian sample path.

\begin{theorem}
	Let $\{\Pi^n\}$ be a sequence of partitions of $[0, T]$ such that $\sum_n |\Pi^n| < \infty$. Then
	\begin{align*}
		\lim_{n \rightarrow \infty} V_t^2(B(\omega), \Pi^n) = t, \quad \text{for all } t \in [0, T] \quad \mathbb{P}\text{-a.s.}
	\end{align*}
\end{theorem}
\begin{proof}
	We assume that $t$ belongs to the partitions $\Pi^n$; the general case follows by a standard $\epsilon$-argument.\\
	\textbf{1. $L^2$-convergence.} By independence of the increments and $\mathbb{E}[(B_{t_i} - B_{t_{i-1}})^2] = t_i - t_{i-1}$,
	\begin{align*}
		\mathbb{E}\bigg[ \bigg( \sum_{t_i \in \Pi^n} (B_{t_i} - B_{t_{i-1}})^2 - t \bigg)^2 \bigg]
		&= \mathbb{E}\bigg[ \bigg( \sum_{t_i \in \Pi^n} \big\{ (B_{t_i} - B_{t_{i-1}})^2 - (t_i - t_{i-1}) \big\} \bigg)^2 \bigg]\\
		&= \sum_{t_i \in \Pi^n} \mathbb{E}\big[ ((B_{t_i} - B_{t_{i-1}})^2 - (t_i - t_{i-1}))^2 \big]\\
		&= \sum_{t_i \in \Pi^n} \var\big( (B_{t_i} - B_{t_{i-1}})^2 \big).
	\end{align*}
	Since $B_{t_i} - B_{t_{i-1}} \sim N(0, t_i - t_{i-1})$, it follows that $\var((B_{t_i} - B_{t_{i-1}})^2) = 2(t_i - t_{i-1})^2$. Hence
	\begin{align*}
		\mathbb{E}\bigg[ \bigg( \sum_{t_i \in \Pi^n} (B_{t_i} - B_{t_{i-1}})^2 - t \bigg)^2 \bigg]
		= 2 \sum_{t_i \in \Pi^n} (t_i - t_{i-1})^2 \le 2 |\Pi^n| \, t,
	\end{align*}
	which converges to zero as $n \rightarrow \infty$. In particular, the quadratic variation $\langle B(\omega) \rangle_t$ exists for every $t \in [0, T]$ as an $L^2$ limit, without any additional assumptions on the partitions.\\
	\textbf{2. Almost sure convergence.} We have $\mathbb{E}[V_t^2(B, \Pi^n)] = t$. Thus, by Tchebysheff's inequality, for every $\epsilon > 0$,
	\begin{align*}
		\mathbb{P}\big[ |V_t^2(B, \Pi^n) - t| > \epsilon \big] \le \frac{\var(V_t^2(B, \Pi^n))}{\epsilon^2}.
	\end{align*}
	Since a Brownian sample path has independent increments,
	\begin{align*}
		\var(V_t^2(B, \Pi^n)) = \sum_{t_i \in \Pi^n} \var\big( (B_{t_i} - B_{t_{i-1}})^2 \big) = 2 \sum_{t_i \in \Pi^n} (t_i - t_{i-1})^2 \le 2T |\Pi^n|,
	\end{align*}
	so that
	\begin{align*}
		\mathbb{P}\big[ |V_t^2(B, \Pi^n) - t| > \epsilon \big] \le \frac{2T}{\epsilon^2} |\Pi^n|.
	\end{align*}
	Since $\sum_n |\Pi^n| < \infty$, the Borel--Cantelli lemma yields
	\begin{align*}
		\mathbb{P}\big[ |V_t^2(B, \Pi^n) - t| > \epsilon \text{ infinitely often} \big] = 0.
	\end{align*}
	This shows that for every $t \in [0, T]$ there exists a null set $N_t$ such that $\lim_n V_t^2(B(\omega), \Pi^n) = t$ for all $\omega \notin N_t$.\\
	\textbf{3. Simultaneous null set.} Put $N := \bigcup_{t \in [0, T] \cap \mathbb{Q}} N_t$; this set has measure zero and
	\begin{align*}
		\lim_{n \rightarrow \infty} V_t^2(B(\omega), \Pi^n) = t \quad \text{for all } t \in [0, T] \cap \mathbb{Q}, \ \omega \notin N.
	\end{align*}
	For any $t \in [0, T]$ there are sequences $\{t_n\}, \{s_n\} \in [0, T] \cap \mathbb{Q}$ with $t_n \uparrow t$ and $s_n \downarrow t$. By monotonicity of $t \mapsto V_t^2(B, \Pi^n)$, there exists $n^*(\omega)$ such that for all $\omega \notin N$ and $n \ge n^*(\omega)$,
	\begin{align*}
		t_n - \epsilon \le V_{t_n}^2(B(\omega), \Pi^n) \le V_t^2(B(\omega), \Pi^n) \le V_{s_n}^2(B(\omega), \Pi^n) \le s_n + \epsilon.
	\end{align*}
	This proves the result.
\end{proof}

We write $C^{1,2}(\mathbb{R}^m \times \mathbb{R}^n)$ for the class of all continuous functions $F : \mathbb{R}^m \times \mathbb{R}^n \rightarrow \mathbb{R}$ whose first partial derivatives in $\mathbb{R}^m$ and whose first and second partial derivatives in $\mathbb{R}^n$ exist and are continuous. For a partition $\Pi$ and a continuous function $X$ we denote the increments along $\Pi$ by $\Delta_i X^j := X^j_{t_{i+1}} - X^j_{t_i}$.

\begin{theorem}[Basic It\^o formula]
	Let $X : [0, T] \rightarrow \mathbb{R}^n$ be continuous, of continuous quadratic variation, and let $A : [0, T] \rightarrow \mathbb{R}^m$ be continuous and of bounded variation. Let $F : \mathbb{R}^m \times \mathbb{R}^n \rightarrow \mathbb{R}$ be of class $C^{1,2}$. Then
	\begin{align*}
		F(A_t, X_t) = F(A_0, X_0) &+ \int_0^t \nabla_a F(A_s, X_s)\, dA_s + \int_0^t \nabla_x F(A_s, X_s)\, dX_s\\
		&+ \frac{1}{2} \sum_{j, k = 1}^n \int_0^t \frac{\partial^2}{\partial x^j \partial x^k} F(A_s, X_s)\, d\langle X^j, X^k \rangle_s
	\end{align*}
	with
	\begin{align*}
		\int_0^t \nabla_x F(A_s, X_s)\, dX_s := \lim_n \sum_{t_i} \nabla_x F(A_{t_i}, X_{t_i}) \Delta_i X.
	\end{align*}
\end{theorem}

\begin{proposition}
	Let $X$ be real-valued, continuous and of continuous quadratic variation and $f \in C^1$. Then the mapping $t \mapsto f(X_t)$ is of continuous quadratic variation and
	\begin{align*}
		\langle f(X) \rangle_t = \int_0^t (f'(X_s))^2\, d\langle X \rangle_s.
	\end{align*}
\end{proposition}

\begin{corollary}
	Let $X$ be real-valued, continuous and of continuous quadratic variation and $g \in C^1$ with $G' = g$. Then the process
	\begin{align*}
		Y_t := \int_0^t g(X_s)\, dX_s
	\end{align*}
	has quadratic variation
	\begin{align*}
		\langle Y \rangle_t = \int_0^t g^2(X_s)\, d\langle X \rangle_s.
	\end{align*}
\end{corollary}
...
\begin{definition}
	A stochastic process $M = (M_t)$ on a filtered probability space $(\Omega, \mathcal{F}, (\mathcal{F}_t), \mathbb{P})$ is called a \textbf{local martingale} if there exists a \textbf{localising sequence} of stopping times $\{\tau_n\}$, that is, a sequence of stopping times $\{\tau_n\}$ such that
	\begin{itemize}
		\item $\tau_n \uparrow \infty$ a.s.
		\item the stopped process $M^{\tau_n} = M_{\tau_n \wedge \cdot}$ is a martingale w.r.t.\ $(\mathcal{F}_t)$ for every $n \in \mathbb{N}$.
	\end{itemize}
	The space of all continuous local martingales is denoted $\mathcal{M}^c_{loc}$; the space of continuous local martingales starting in zero is denoted $\mathcal{M}^{c,0}_{loc}$.
\end{definition}
\begin{remark}
Here the superscript $c$ means \textbf{continuous}; it does not mean uniformly integrable.
\begin{enumerate}
	\item The class $\mathcal{M}^c_{loc}$ is stable under stopping.
	\item A common variant of the definition requires $(M - M_0)^{\tau_n}$ to be a uniformly integrable martingale for every $n$. This is useful in the construction of stochastic integrals.
\end{enumerate}	
\end{remark}
...
\begin{lemma}
	Let $M$ be a continuous local martingale with respect to a filtration that satisfies the usual conditions starting in $0$. If $M$ is of bounded variation (that is, $V_t(M(\omega)) < \infty$ a.s.\ for all $t \ge 0$), then $\mathbb{P}[M_t = 0 \text{ for all } t \ge 0] = 1$.
\end{lemma}

\begin{proof}
	By the continuity of the sample paths, it is enough to prove that $\mathbb{P}[M_t = 0] = 1$ for all $t \ge 0$. To this end, we proceed in three steps.

	\textbf{Step 1.} Let us first assume that $M$ is a continuous, \textbf{square integrable} martingale with \textbf{uniformly bounded variation}, i.e., $V_t(M(\omega)) \le C$ for almost all $\omega \in \Omega$. Then, for any partition $0 = t_0 < t_1 < \dots < t_n = t$, writing $\Delta_i M = M_{t_i} - M_{t_{i-1}}$ and using $M_0 = 0$ (so that $\sum_i \Delta_i M = M_t$),
	\begin{align*}
		\mathbb{E} M_t^2 = \mathbb{E} \left[ \sum_i \Delta_i M \right]^2 = \sum_i \mathbb{E}[\Delta_i M]^2 \le C\, \mathbb{E}\left[ \sup_i |\Delta_i M| \right],
	\end{align*}
	where the second equality is the orthogonality of increments and the inequality uses $\sum_i |\Delta_i M| \le V_t(M) \le C$. Moreover, since $|\Delta_i M| \le 2 \sup_{0 \le u \le t} |M_u|$, Doob's $L^2$ maximal inequality gives
	\begin{align*}
		\mathbb{E}\left[ \sup_{n \in \mathbb{N},\, t_i \in \Pi^n} |\Delta_i M|^2 \right] \le 4\, \mathbb{E}\left[ \sup_{0 \le u \le t} |M_u|^2 \right] \le 16\, \mathbb{E}|M_t|^2 < \infty.
	\end{align*}
	In particular, $\sup_{n \in \mathbb{N},\, t_i \in \Pi^n} |\Delta_i M| \in L^2$. 
	By continuity, $\sup_i |\Delta_i M| \to 0$ a.s.\ if $|\Pi^n| \to 0$.
	Hence, by dominated convergence $\mathbb{E} M_t^2 = 0$ and $\mathbb{P}(M_t = 0) = 1$.

	\textbf{Step 2.} Let us now assume that $M$ is a continuous square integrable martingale of \textbf{bounded} variation and put
	\begin{align*}
		\tau^n := \inf\{t \ge 0 : V_t(M(\omega)) > n\}.
	\end{align*}
	Then
	\begin{align*}
		\{t \le \tau^n\} = \{V_t(M(\omega)) \le n\} \in \mathcal{F}_t.
	\end{align*}
	Thus $\tau^n$ is an optional time and hence a stopping time because the filtration satisfies the usual conditions. By optional sampling, the stopped process $M^{\tau^n}$ is a martingale; it has uniformly bounded variation $V_\cdot(M^{\tau^n}) \le n$, so by Step 1 $\mathbb{E} M^2_{\tau^n \wedge t} = 0$, i.e.\ $M_{\tau^n \wedge t} = 0$ a.s.\ for every $n$. Since $\tau^n \uparrow \infty$ a.s., we have $M_{\tau^n \wedge t} \to M_t$ a.s., hence $M_t = 0$ a.s.\ and $\mathbb{E} M_t^2 = 0$.

	\textbf{Step 3.} Let us finally assume that $M$ is a continuous local martingale with localising sequence $\{\tau^n\}$. We may w.l.o.g.\ assume that $M^{\tau^n}$ is a bounded martingale; else we can define
	\begin{align*}
		\sigma^n := \tau^n \wedge \inf\{t \ge 0 : |M_t| \ge n\},
	\end{align*}
	which is again a localising sequence. Hence $\mathbb{P}(M_{t \wedge \tau^n} = 0) = 1$. Since $\tau^n \uparrow \infty$ a.s.\ dominated convergence yields $\mathbb{P}(M_t = 0) = 1$.
\end{proof}



\begin{lemma}
	Let $M$ be a martingale bounded by $K$ with $M_0 = 0$, and let $V_t^4(M, \Pi)$ denote the sum of the fourth moments of the increments along the partition $\Pi$ of $[0, t]$. Then, for any partition $\Pi = \{0 = t_0 < t_1 < \dots < t_m = t\}$ of $[0, t]$,
	\begin{align*}
		\mathbb{E}\big[ V_t^2(M, \Pi) \big]^2 \le 6 K^4.
	\end{align*}
	If $M$ is in addition continuous, then
	\begin{align*}
		\lim_{|\Pi^n| \rightarrow 0} \mathbb{E} V_t^4(M, \Pi^n) = 0.
	\end{align*}
\end{lemma}

\begin{theorem}
	Let $M$ be a continuous local square integrable martingale with respect to a filtration that satisfies the usual conditions. Let $M_0 = 0$. Then there exists an almost surely unique continuous process $\langle M \rangle$ (called the \textbf{bracket process}, \textbf{compensator} or \textbf{quadratic variation process}) such that
	\begin{itemize}
		\item $\langle M \rangle_0 = 0$ and $\langle M \rangle$ is non-decreasing
		\item $M^2 - \langle M \rangle$ is a continuous local martingale
		\item for any sequence of partitions $\{\Pi^n\}$ of $[0, t]$ with $|\Pi^n| \rightarrow 0$,
		\begin{align*}
			V_t^2(M(\omega), \Pi^n) \rightarrow \langle M \rangle_t \quad \text{uniformly on compact time intervals in probability (ucp)}
		\end{align*}
	\end{itemize}
\end{theorem}
\begin{proof}
	The existence of the bracket process for a bounded continuous martingale was established in the construction above; it remains to reduce the general case to the bounded one. Let $M$ be a continuous local martingale with localising sequence $\{\tau^n\}$ and put $M_t^n := M_{\tau^n \wedge t}$; w.l.o.g.\ $M^n$ is bounded (otherwise replace $\tau^n$ by $\sigma^n := \tau^n \wedge \inf\{t \ge 0 : |M_t| \ge n\}$). Let $\langle M^n \rangle$ be the bracket process constructed for the bounded martingale $M^n$. By construction and uniqueness of the bracket process, for $n \ge m$,
	\begin{align*}
		\langle M^n \rangle \mathbbm{1}_{[0, \tau^m]} = \langle M^m \rangle \mathbbm{1}_{[0, \tau^m]},
	\end{align*}
	and $\langle M^n \rangle_t$ is increasing in $n$ since $\tau^n \uparrow \infty$. Define
	\begin{align*}
		\langle M \rangle_t := \lim_{n \rightarrow \infty} \langle M^n \rangle_t,
	\end{align*}
	which is continuous (pointwise limit of continuous functions) and finite on $[0, \tau^m]$ for each $m$. Moreover,
	\begin{align*}
		(M^2 - \langle M \rangle)^{\tau^m \wedge \cdot} = ((M^m)^2 - \langle M^m \rangle)^{\tau^m \wedge \cdot}
	\end{align*}
	is a martingale (the right-hand side is a martingale, and remains one after stopping by optional sampling), so $M^2 - \langle M \rangle$ is a continuous local martingale. The properties $\langle M \rangle_0 = 0$ and monotonicity follow from the construction, and uniqueness follows again from the fact that a continuous local martingale of bounded variation starting in zero is a.s.\ constant.\\
	It remains to prove the convergence in probability of $V_t^2(M, \Pi^m)$ to $\langle M \rangle_t$. Recall $V_t^2(M^n, \Pi^m) \rightarrow \langle M^n \rangle_t$ in $L^2$, hence in probability, as $m \rightarrow \infty$, and $M^n \equiv M$ on $[0, \tau^n]$, so $V^2(M^n, \Pi^m) \equiv V^2(M, \Pi^m)$ there. Since $\{\tau^n\}$ is localising, $\lim_n \mathbb{P}[\tau^n < t] = 0$; choose $n(\epsilon)$ with $\mathbb{P}[\tau^n \le t] \le \epsilon$ for $n \ge n(\epsilon)$. Then for $m$ large,
	\begin{align*}
		\Big\{ \sup_{0 \le t \le T} |V_t^2(M, \Pi^m) - \langle M \rangle_t| < \epsilon \Big\}
		\supseteq\ &\Big\{ \sup |V_t^2(M, \Pi^m) - V_t^2(M^{n(\epsilon/3)}, \Pi^m)| < \epsilon/3 \Big\}\\
		\cap\ &\Big\{ \sup |V_t^2(M^{n(\epsilon/3)}, \Pi^m) - \langle M^{n(\epsilon/3)} \rangle_t| < \epsilon/3 \Big\}\\
		\cap\ &\Big\{ \sup |\langle M^{n(\epsilon/3)} \rangle_t - \langle M \rangle_t| < \epsilon/3 \Big\},
	\end{align*}
	and each of the three sets on the right has probability close to one for $m$ large. Hence the left-hand event has probability at least $1 - \epsilon$, which gives $V_t^2(M, \Pi^m) \rightarrow \langle M \rangle_t$ ucp.
\end{proof}

The bracket process gives us a measure to compute the stochastic integral under some measurability conditions. 


\section{Stochastic Integral}
\begin{definition}[Simple/Elementary Process]
	We call a process $\phi = (\phi_t)$ \textbf{simple/elementary} if it is of the form
	\begin{equation*}
		\phi_t(\omega) = \sum_j e_j(\omega) \mathbf{1}_{(t_j, t_{j+1}]}(t)
	\end{equation*}
	where $0 = t_0 < t_1 < t_2 < \cdots$, where $e_j$ is $\mathcal{F}_{t_j}$-measurable and $\sup_{t,\omega} |e_j(\omega)| < \infty$. The class of all such functions is denoted $\mathcal{L}_0$.
\end{definition}

\begin{lemma}[It\^{o} Isometry]
	Let $\phi$ be an elementary function. Then
	\begin{equation*}
		\mathbb{E}\left[\left(\int_0^T \phi_t(\omega)\, dB_t\right)^2\right] = \mathbb{E}\left[\int_0^T \phi_t^2(\omega)\, dt\right].
	\end{equation*}
\end{lemma}
\begin{proof}
	Let $\Delta_j B := B_{t_{j+1}} - B_{t_j}$ and fix $i \leq j \in \mathbb{N}$. Since $e_i$ and $e_j$ are measurable with respect to $\mathcal{F}_{t_i}$ and $\mathcal{F}_{t_j}$, respectively, we have for $i = j$ that
	\begin{align*}
		\mathbb{E}[e_i e_j \Delta_i B \Delta_j B]
		&= \mathbb{E}\!\left[\mathbb{E}\!\left[e_j^2 (\Delta_j B)^2 \mid \mathcal{F}_{t_j}\right]\right] \\
		&= \mathbb{E}\!\left[e_j^2\, \mathbb{E}\!\left[(\Delta_j B)^2 \mid \mathcal{F}_{t_j}\right]\right] \\
		&= \mathbb{E}\!\left[e_j^2\right](t_{j+1} - t_j),
	\end{align*}
	while for $i < j$ we obtain
	\begin{align*}
		\mathbb{E}[e_i e_j \Delta_i B \Delta_j B]
		&= \mathbb{E}\!\left[\mathbb{E}\!\left[e_i e_j \Delta_i B \Delta_j B \mid \mathcal{F}_{t_j}\right]\right] \\
		&= \mathbb{E}\!\left[e_i \Delta_i B\, e_j\, \mathbb{E}\!\left[\Delta_j B \mid \mathcal{F}_{t_j}\right]\right] \\
		&= 0.
	\end{align*}
	Hence
	\begin{align*}
		\mathbb{E}\!\left[\left(\int_0^T \phi_t(\omega)\, dB_t\right)^2\right]
		&= \sum_{i,j} \mathbb{E}[e_i e_j \Delta_i B \Delta_j B] \\
		&= \sum_j \mathbb{E}\!\left[e_j^2\right](t_{j+1} - t_j) \\
		&= \mathbb{E}\!\left[\int_0^T \phi_t^2(\omega)\, dt\right]. \qedhere
	\end{align*}
\end{proof}

\quad\\
1. What kind of process can be approximated by simple process? 
The question is easy for Brownian motion but not obvious for local martingale. \\
2. Can we define the integral by the measure theoritic induction like before? (by Ito isometry)

\begin{definition}[$\mathcal{L}_2$]
	Let $\mathcal{L}_2$ be the class of all jointly measurable and $(\mathcal{F}_t)$-adapted functions $f_t(\omega) : [0,\infty) \times \Omega \to \mathbb{R}$ such that
	\begin{equation*}
		\|f\|^2_{L^2_T(\lambda \otimes \mathbb{P})} := \mathbb{E}\left[\int_0^T f_t^2(\omega)\, dt\right] < \infty
	\end{equation*}
	for all $T > 0$. For any $f, g \in \mathcal{L}_2$, we set
	\begin{equation*}
		d(f,g) := \sum_{n=1}^{\infty} 2^{-n} \min\!\left\{1,\, \|f - g\|_{L^2_n(\lambda \otimes \mathbb{P})}\right\}.
	\end{equation*}
\end{definition}

\quad\\
Adaptedness is tricky, progressive measurable, proposition 1.1.9\\
Does composition of L2 convergence and a.s. convergence implies L2 convergence?\\
The left-continuous assumptions
