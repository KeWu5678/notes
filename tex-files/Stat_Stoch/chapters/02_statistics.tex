...
\begin{method}[method of moments]
\begin{enumerate}
...
For estimation we can see that there exists a problem. The basis
\[
	\{\psi_j\}_j = (1, x, \dots, x^{q - 1}, \psi_0(x), \dots, \psi_{k-1}(x))
\]
is useful for theory but not for practice, because the basis are strongly correlated. A remedy for this is called B-splines. The idea is $\psi_j(x)$ has a support only on $q$ neighbour pieces. Nice numeric for B-splines.
...
As seen in the previous section, we focus on the expectation $\mathbb{E}[Y_i]$, which is only meaningful under a distributional assumption on $Y_i$. When Nelder and Wedderburn (1972) introduced generalized linear models, they used exponential families for this purpose.

\begin{definition}[Generalized Linear Model]
On a product model $(\mathcal{X}^n, \mathcal{F}^{\otimes n})$, a generalized linear model (GLM) with $n$ independent observations $Y_1, \dots, Y_n$ is given when the marginal distributions of $Y_i$ belong to a natural exponential family with densities
\begin{equation}
	\frac{d\mathbb{P}_{\eta_i}^{Y_i}}{d\mu}(y_i) = \exp\left(\frac{\eta_i y_i - \zeta(\eta_i)}{\varphi}\right) c(y_i, \varphi), \quad i = 1, \dots, n
\end{equation}
with respect to a dominating measure $\mu$, with dispersion parameter $\varphi > 0$,
\begin{equation*}
	\eta_i \in \Xi = \left\{ \eta \in \mathbb{R} : \int_{\mathcal{X}} e^{\eta y / \varphi}\, c(y, \varphi)\, \mu(dy) \in (0, \infty) \right\} \subset \mathbb{R}
\end{equation*}
for all $i$, and known functions $\zeta \colon \Xi \to \mathbb{R}$ and $c \colon \mathcal{X} \times \mathbb{R}_+ \to \mathbb{R}_+$ with $\zeta''(\eta) > 0$ for all interior points $\eta \in \Xi^\circ$. Define $\rho(\eta_i) := \mathbb{E}_{\eta_i}[Y_i]$.

For an unknown parameter vector $\beta \in \mathbb{R}^k$, a design matrix $X \in \mathbb{R}^{n \times k}$, and a bijective, continuously differentiable function $g \colon \mathbb{R} \to \mathbb{R}$, we require
\begin{equation}
	\begin{pmatrix} g(\rho(\eta_1)) \\ \vdots \\ g(\rho(\eta_n)) \end{pmatrix} = X\beta.
\end{equation}
The function $g$ is called the \textbf{link function}. If $\rho = g^{-1}$, then $g$ is the \textbf{canonical link}, and $(\eta_1, \dots, \eta_n)^T = X\beta$.
\end{definition}

\begin{remark}[Dispersion parameter]
While $\beta$ is the parameter of interest, $\varphi$ is treated as a nuisance parameter. For fixed $\varphi$, $Y_i$ follows a natural exponential family in $T(y) = y/\varphi$. The parameter $\varphi$ arises from the exponential family and is typically known, related to the variance of the distribution. Hence the name \emph{dispersion parameter}: dispersion measures the spread of individual values around the mean.
\end{remark}

\begin{example}[Gaussian linear model as GLM]
The ordinary linear model $Y = X\beta + \varepsilon$ with i.i.d.\ errors $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$, known variance $\sigma^2 > 0$, and row vectors $x_1, \dots, x_n \in \mathbb{R}^k$ of the design matrix $X$, is a GLM. Indeed, $Y_i \sim \mathcal{N}(x_i\beta, \sigma^2)$ and the density can be written as
\begin{equation*}
	\frac{d\mathbb{P}_{\eta_i}^{Y_i}}{d\mu}(y_i) = \exp\!\left(\frac{x_i\beta \cdot y_i - (x_i\beta)^2/2}{\sigma^2}\right) \frac{1}{\sqrt{2\pi\sigma^2}} \exp\!\left(-\frac{y_i^2}{2\sigma^2}\right), \quad i = 1, \dots, n.
\end{equation*}
The natural parameter is $\eta_i = x_i\beta$, the dispersion parameter is $\varphi = \sigma^2$, and $c(y_i, \sigma^2) = \frac{1}{\sqrt{2\pi\sigma^2}} \exp\!\left(-\frac{y_i^2}{2\sigma^2}\right)$. Since $\zeta(\eta) = \eta^2/2$, we have $\zeta''(\eta) \equiv 1 > 0$ and $\rho(\eta_i) = \mathbb{E}[Y_i] = \zeta'(\eta_i) = \eta_i = x_i\beta$. Thus $g = \mathrm{id}_{\mathbb{R}}$ is the canonical link, and $(\eta_1, \dots, \eta_n)^T = X\beta$.
\end{example}

\begin{remark}[Link function and dispersion parameter] ~
\begin{itemize}
	\item The link function can be freely chosen for modelling purposes, as long as it is bijective and continuously differentiable.
	\item In the Gaussian example above, $\sigma^2$ is assumed known. If unknown, it can be calibrated from the data.
\end{itemize}
\end{remark}

\begin{lemma}[Moments in the GLM]
In the generalized linear model,
\begin{equation*}
	\mathbb{E}_{\beta, \varphi}[Y_i] = \zeta'(\eta_i), \quad \mathrm{Var}_{\beta, \varphi}(Y_i) = \varphi \, \zeta''(\eta_i), \quad i = 1, \dots, n.
\end{equation*}
\end{lemma}

\begin{proof}
Let $\bar{\zeta}(\eta) = \zeta(\eta)/\varphi$, $T_i := Y_i / \varphi$, and $\bar{\rho}(\eta) = \rho(\eta)/\varphi$. From the properties of natural exponential families:
\begin{equation*}
	\mathbb{E}_{\beta, \varphi}[Y_i]/\varphi = \mathbb{E}_{\beta, \varphi}[T_i] = \bar{\rho}(\eta_i) = \bar{\zeta}'(\eta_i) = \zeta'(\eta_i)/\varphi,
\end{equation*}
giving $\mathbb{E}_{\beta,\varphi}[Y_i] = \zeta'(\eta_i)$. Similarly,
\begin{equation*}
	\mathrm{Var}_{\beta, \varphi}[Y_i]/\varphi^2 = \mathrm{Var}_{\beta, \varphi}[T_i] = \bar{\rho}'(\eta_i) = \bar{\zeta}''(\eta_i) = \zeta''(\eta_i)/\varphi,
\end{equation*}
giving $\mathrm{Var}_{\beta,\varphi}[Y_i] = \varphi \, \zeta''(\eta_i)$.
\end{proof}

\subsubsection*{MLE in the GLM}

To estimate the unknown parameter vector $\beta$, we use the maximum likelihood approach. Since $\mathrm{Var}_{\beta,\varphi}(Y_i) = \varphi \zeta''(\eta_i) > 0$ and $\varphi > 0$, the function $\zeta'$ is strictly monotone increasing, hence $\rho$ is invertible. Since $g$ is also invertible, define $\psi := (g \circ \rho)^{-1}$. Let $x_i \in \mathbb{R}^k$ denote the $i$-th row of $X$, then
\begin{equation*}
	\eta_i = \rho^{-1} \circ g^{-1}(x_i^T\beta) = (g \circ \rho)^{-1}(x_i^T\beta) = \psi(x_i^T\beta).
\end{equation*}
The log-likelihood becomes
\begin{equation}
	\log L(\beta, \varphi; y) = \sum_{i=1}^n \left( \frac{\psi(x_i^T\beta)\, y_i - \zeta\big(\psi(x_i^T\beta)\big)}{\varphi} + \log c(y_i, \varphi) \right).
\end{equation}
Differentiating with respect to $\beta$ and using $\rho = \zeta'$, we obtain the score equation:
\begin{equation} \label{eq:GLM_score}
	\nabla_\beta \log L(\hat{\beta}, \varphi; y) = \frac{1}{\varphi} \sum_{i=1}^n \big(y_i - \rho(\psi(x_i^T\hat{\beta}))\big)\, \psi'(x_i^T\hat{\beta})\, x_i = 0.
\end{equation}

\begin{remark}[MLE in the GLM] ~
\begin{enumerate}
	\item Typically $\hat{\beta}$ has no closed-form solution and must be computed numerically. \emph{Fisher's scoring method} is one such procedure, which requires the Fisher information.
	\item If $g$ is not the canonical link, a solution of \eqref{eq:GLM_score} is not necessarily a maximum likelihood estimator.
\end{enumerate}
\end{remark}
...
Gram matrix:
\[
	\mathbf{X} \mathbf{X}^T : \langle X_i, X_j \rangle_{i, j},
	\qquad
	\mathbf{X} \mathbf{X}^T  = \sum_{j = 1}^n \hat{\sigma}^2 \hat{v}_j \hat{v_j}^T .
\]
