...
\paragraph{space of the periodic functions}
...
\paragraph{back to mollification:}
...
\paragraph{The Fourier series $\mathcal{F}$ and $\mathcal{F}^*$:}
...
\paragraph{Completeness via approximate identity}
\begin{lemma}
...
\paragraph{The general idea of Sobolev space and $H^m(\mathbb{R}^n)$, $H^m(\mathbb{T}^n)$} 
\quad\par
...
\paragraph{The Sobolev embedding theorem}
\begin{example}
\end{example}
...
\paragraph{The Compact Inclusion}
\begin{lemma}
...
\subsection{Application: Radon Transform}
\begin{definition}[Radon transform]
Let $f:\mathbb{R}^d\rightarrow \mathbb{R}$ be integrable on every affine hyperplane. For $w\in S^{d-1}$ and $b\in \mathbb{R}$, define the hyperplane
\begin{align*}
	H_{w,b}:= \{x\in \mathbb{R}^d: w\cdot x=b\}.
\end{align*}
The Radon transform of $f$ is the function on $S^{d-1}\times \mathbb{R}$ given by
\begin{align*}
	\mathcal{R}f(w,b):= \int_{H_{w,b}} f(x)\,dS(x),
\end{align*}
where $dS$ denotes the $(d-1)$-dimensional Euclidean surface measure on the hyperplane. Equivalently, in the notation of Helgason's \emph{The Radon Transform}, this is the integral of $f$ over each hyperplane $\xi\subset \mathbb{R}^d$.
\end{definition}

\begin{remark}[Parametrization by normal direction and offset]
The pair $(w,b)$ is not a point in the original domain. It parametrizes the hyperplane with unit normal $w$ and signed distance $b$ from the origin. The same hyperplane is represented twice:
\begin{align*}
	H_{w,b}=H_{-w,-b},
\end{align*}
hence
\begin{align*}
	\mathcal{R}f(w,b)=\mathcal{R}f(-w,-b).
\end{align*}
\end{remark}

\begin{definition}[Dual Radon transform]
For a function $\varphi:S^{d-1}\times \mathbb{R}\rightarrow \mathbb{R}$, the dual Radon transform is
\begin{align*}
	\mathcal{R}^*\varphi(x):= \int_{S^{d-1}}\varphi(w,w\cdot x)\,dw.
\end{align*}
For fixed $x$, the offset variable is set to $b=w\cdot x$ because these are exactly the hyperplanes $H_{w,b}$ passing through $x$.
\end{definition}

\begin{theorem}[Fourier slice theorem]
Let $f\in L^1(\mathbb{R}^d)$, then for every $w\in S^{d-1}$ and $s\in \mathbb{R}$,
\begin{align*}
	\widehat{f}(sw)
	= \int_{\mathbb{R}}\mathcal{R}f(w,b)e^{-isb}\,db.
\end{align*}
That is, the restriction of the $d$-dimensional Fourier transform of $f$ to the line $\{sw:s\in \mathbb{R}\}$ is the one-dimensional Fourier transform of $\mathcal{R}f(w,\cdot)$ in the offset variable. This is Helgason's equation (4) in Chapter I, Section 2.
\end{theorem}
\begin{proof}
Fix $w\in S^{d-1}$. Decompose $\mathbb{R}^d$ into the parallel hyperplanes $H_{w,b}=\{x:w\cdot x=b\}$. By Fubini's theorem, equivalently the coarea formula for the map $x\mapsto w\cdot x$, we have
\begin{align*}
	\widehat{f}(sw)
	&= \int_{\mathbb{R}^d}f(x)e^{-is(w\cdot x)}\,dx\\
	&= \int_{\mathbb{R}}\int_{H_{w,b}} f(x)e^{-is(w\cdot x)}\,dS(x)\,db\\
	&= \int_{\mathbb{R}}e^{-isb}\int_{H_{w,b}} f(x)\,dS(x)\,db\\
	&= \int_{\mathbb{R}}\mathcal{R}f(w,b)e^{-isb}\,db.
\end{align*}
\end{proof}

\begin{theorem}[Dual Radon inversion, Solmon]
Let $\varphi\in \mathcal{S}(S^{d-1}\times \mathbb{R})$ be even, i.e.
\begin{align*}
	\varphi(-w,-b)=\varphi(w,b).
\end{align*}
Then
\begin{align*}
	\gamma_d\,\mathcal{R}\{(-\Delta)^{(d-1)/2}\mathcal{R}^*\varphi\}
	= \varphi,
	\qquad
	\gamma_d=\frac{1}{2(2\pi)^{d-1}},
\end{align*}
\end{theorem}
\begin{proof}
The proof is most transparent after taking the one-dimensional Fourier transform in the offset variable $b$. For $u=\mathcal{R}^*\varphi$, the Fourier transform of $u$ in $\mathbb{R}^d$ receives contributions only from the two sphere directions parallel to the frequency. Writing the sphere variable as $\theta$, we compute distributionally. For fixed $\theta\in S^{d-1}$, write $x=b\theta+y$ with $y\in \theta^\perp$. Then
\begin{align*}
	\widehat{u}(\sigma w)
	&= \int_{\mathbb{R}^d} e^{-i\sigma w\cdot x}
		\int_{S^{d-1}}\varphi(\theta,\theta\cdot x)\,d\theta\,dx\\
	&= \int_{S^{d-1}}\int_{\mathbb{R}}\int_{\theta^\perp}
		\varphi(\theta,b)e^{-i\sigma w\cdot(b\theta+y)}\,dS(y)\,db\,d\theta\\
	&= \int_{S^{d-1}}
		\left(\int_{\theta^\perp}e^{-i\sigma w\cdot y}\,dS(y)\right)
		\left(\int_{\mathbb{R}}\varphi(\theta,b)e^{-i\sigma(w\cdot\theta)b}\,db\right)d\theta\\
	&= (2\pi)^{d-1}\int_{S^{d-1}}
		\delta_{\theta^\perp}(\sigma w)\,
		\mathcal{F}_b\varphi(\theta,\sigma w\cdot\theta)\,d\theta\\
	&= (2\pi)^{d-1}|\sigma|^{-(d-1)}
		\left(\mathcal{F}_b\varphi(w,\sigma)+\mathcal{F}_b\varphi(-w,-\sigma)\right)\\
	&=2(2\pi)^{d-1}|\sigma|^{-(d-1)}\mathcal{F}_b\varphi(w,\sigma),
\end{align*}
where $\delta_{\theta^\perp}$ is the delta distribution supported on the line spanned by $\theta$, and the factor $|\sigma|^{-(d-1)}$ is the Jacobian from restricting the sphere integral to the two points $\theta=\pm w$. The last equality uses the evenness condition $\varphi(-w,-b)=\varphi(w,b)$. Applying $(-\Delta)^{(d-1)/2}$ multiplies this by $|\sigma|^{d-1}$. Taking the Radon transform and again Fourier transforming in $b$ gives
\begin{align*}
	\mathcal{F}_b\mathcal{R}\{(-\Delta)^{(d-1)/2}\mathcal{R}^*\varphi\}(w,\sigma)
	=2(2\pi)^{d-1}\mathcal{F}_b\varphi(w,\sigma).
\end{align*}
Multiplication by $\gamma_d$ and inversion of the one-dimensional Fourier transform in $b$ gives the claim. Solmon proves this rigorously for even Schwartz functions on $S^{d-1}\times\mathbb{R}$.
\end{proof}

\begin{remark}[Filtered backprojection]
The operator $\mathcal{R}^*\mathcal{R}$ is not the identity. It backprojects the hyperplane integrals and produces a smoothed version of $f$. The correction is the filtering step in the inversion formula
\begin{align*}
	f = \gamma_d (-\Delta)^{(d-1)/2}\mathcal{R}^*\mathcal{R}f,
\end{align*}
\end{remark}
