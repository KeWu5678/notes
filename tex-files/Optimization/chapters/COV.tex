\chapter{Calculus of Variation and Optimal Control}
\section{Weak Solution}
\paragraph{Weak and Strong extrema}
...
\begin{equation}\tag{(P1)}\label{prob:P1}
\begin{aligned}
	& \inf_{y \in U} J(y, \dot{y}):= \int_a^b L(y, \dot{y}, t) dt,  \\
	& \text{s.t.}\\
...
\begin{remark}[physical interpretation]
	In classical mechanics, $y(t)$ is the position of a particle, $\dot{y}(t)$ its velocity, and the Lagrangian is the difference of kinetic and potential energy,
	\begin{equation*}
		L(y, \dot{y}, t) = T(\dot{y}) - V(y) = \tfrac{1}{2}m\dot{y}^2 - V(y).
	\end{equation*}
	The functional $J(y) = \int_a^b L\, dt$ is then the \textbf{action}, and \textbf{Hamilton's principle of stationary action} states that the physical trajectory between the fixed endpoints $y_0, y_1$ is a stationary point of $J$. In this picture the Euler--Lagrange equation recovers Newton's second law: with $L_z = m\dot{y}$ (momentum) and $L_y = -V'(y)$ (force),
	\begin{equation*}
		\frac{d}{dt}L_z = L_y \quad\Longleftrightarrow\quad m\ddot{y} = -V'(y) = F.
	\end{equation*}
	More generally $L_z = \partial L / \partial \dot{y}$ is the \emph{generalised momentum} and $L_y = \partial L / \partial y$ the \emph{generalised force}, so the Euler--Lagrange equation expresses the balance ``rate of change of momentum $=$ force.''
\end{remark}

First we consider the classical case where the admissible sets are at least $\mathcal{C}^1$. We introduce the \textbf{strong and weak extrema}.
\begin{definition}[Strong and Weak Extrema]	
...
\end{aligned}
\end{equation}
	We call $y^*$ \textbf{a local weak extremum} of \eqref{prob:P1} if $y \in C^1([a, b])$ and
	\begin{equation}
...
\end{lemma}
\begin{proof}
	We argue by contradiction. Suppose $y(\bar{x}) \neq 0$ for some $\bar{x}$; without loss of generality $y(\bar{x}) > 0$ (otherwise replace $y$ by $-y$). By continuity of $y$ there exists an interval $[c, d] \subset [a, b]$ with $\bar{x} \in [c, d]$ such that
	\begin{align*}
		y > 0 \quad \text{on } [c, d].
	\end{align*}
	Define
	\begin{align*}
		...
		\eta(x) &= 0 &&\text{otherwise.}
	\end{align*}
	Then $\eta(c) = \eta(d) = 0$ and $\eta'(c) = \eta'(d) = 0$, so $\eta \in \mathcal{C}^1_0([a, b])$. Moreover $\eta > 0$ on $(c, d)$, hence
	\begin{align*}
		\int_a^b y \eta = \int_c^d y\, \eta > 0,
	\end{align*}
	contradicting the hypothesis $\int_a^b y \eta = 0$. Therefore $y \equiv 0$.
\end{proof}
\begin{remark}
The weaker version:
...
\begin{theorem}[necessary condition for weak extrema]
	Assuming $L \in C^2$ w.r.t. $z$.  If $y$ is a weak extrema of the problem \eqref{prob:P1}, then it holds
	\begin{equation}
	\begin{aligned}
		& \frac{d}{dt}L_{z} = L_y\\
		& y(a) = y_0, y(b) = y_1
...
\begin{theorem}[relaxing the assumption on ELE]
	Assuming $L \in C^1$, $y \in C^1$, if $y$ is a weak extrema of \eqref{prob:P1}, then it holds
	\begin{align*}
		& \frac{d}{dt}L_z = L_y\\
		& y(a) = y_0, \quad y(b) = y_1. 
	\end{align*}
...
\begin{equation}\tag{(P2)}\label{prob:P2}
\begin{aligned}
...
\begin{theorem}[the Hamiltonian ODE]
	The 1st order optimality condition of \eqref{prob:P1} is equivalent in the Hamiltonian formlism:
	\begin{align*}
...
\begin{equation}\tag{(P3)}\label{prob:P3}
\begin{aligned}
	& \inf_{y \in U} J(y)\\
	& \text{s.t.}\\
...
\begin{theorem}
Assume $y^*$ is a weak extremum of \eqref{prob:P3} , then there exists a Lagrange multiplier $\lambda^* \in \mathbb{R}$, such that,  $y^*, \lambda^* $ satisfies
\begin{equation}
\begin{aligned}
...
We observe again the unconstrained classical Lagrange problem \eqref{prob:P1}. We want to derive now the second order optimality condition of the Lagrange problem.
...
\begin{theorem}[2nd order necessary optimality condition]
	If $y^*$ is a weak local minimum of \eqref{prob:P1}, then it holds
	\begin{equation}
...
Given the problem \eqref{prob:P1}, assuming
\begin{equation*}
...
\subsection{Necessary Conditions For \texorpdfstring{$C_{PW}^1$}{C1\_PW} Strong Solutions}
...
\begin{theorem}[Weierstrass-Erdmann corner condition for strong extrema]
	Given the problem \eqref{prob:P1} and $y^* \in  C^1_{PW}$ \textbf{strong} extrema of the problem \textbf{(P1.1)}, then $y^*$ satisfies:
	\begin{equation}
	\begin{aligned}
...
\subsection{Sufficient Condition Of The \texorpdfstring{$C^2$}{C2} Solution}
...
\begin{theorem}[Sufficient condition for strong(local) minimum]
	Given \eqref{prob:P1} and $y^* \in \mathcal{S} $ , assuming the slope of extremal $p$ exists in a neighbourhood of $y^*$, if 
	\begin{enumerate}
...
\begin{equation}\tag{(P2.1)}\label{prob:P2.1}
\begin{aligned}
...
\begin{definition}[optimal control]
	$u^*$ is an optimal control of \eqref{prob:P2.1}, if for all feasible control $u  \in C_{PW}([t_0, t_1])^m$, we have
	\begin{align*}
		J(u^*) \le J(u),
	\end{align*}
...
Given \eqref{prob:P2.1}, for $p \in C^1([t_0, t_1])$, define the Hamiltonian:
\begin{equation}
...
\begin{equation}\tag{(P3.1)}\label{prob:P3.1}
\begin{aligned}
...
Given \eqref{prob:P3.1} and define the Hamiltonian:
\begin{equation}
...
\begin{enumerate}
	...
	\end{align*}
	\item $h_1, \dots, h_{n-k} \in \mathcal{C}^1$ have linearly independent gradients $h_1'(x), \dots, h_{n-k}'(x)$ at every $x \in S_1$, so that $S_1$ is a regular manifold of dimension $k$.
		\item $f, L \in \mathcal{C}^1$ jointly in $(x, u)$.
\end{enumerate}
If $u^*$ is the \textbf{global} optimal control and $x^*$ the corresponding optimal trajectory, then there exits $p^* \in C^1([t_0, t_f])^n$, with $p_0^* \le  0$,  $(p^*_0, p^*(t)) \neq 0$,  such that:
\begin{enumerate}
	\item $\dot{p}^* = - \partial_x H|_{x^*, u^*, p^*}$ and $\dot{x}^* = \partial_p H$. 
	\item For a.e.\ $t \in [t_0, t_f]$ and $\forall u \in U$:
	\begin{equation}
		H_{x^*, u^*, p^*, p_0^*}(t) \ge H_{x^*, u, p^*, p_0^*}(t). 
	\end{equation}
	\item $\forall t \in [t_0, t_f]:$
	\begin{equation}
		H_{x^*, u^*, p^*, p_0^*}(t) \equiv \mathrm{const} \quad (\equiv 0 \text{ if } t_f \text{ is free}).
	\end{equation}
...
\begin{equation}\label{eq:needle-var-spatial}
\begin{aligned}
...
\begin{equation}\label{eq:needle-var-terminal}
\begin{aligned}
...
\begin{equation}\tag{(P4.1)}\label{prob:P4.1}
\begin{aligned}
