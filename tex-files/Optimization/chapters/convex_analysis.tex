...
\begin{algorithm}
\caption{Sequential Quadratic Programming (SQP)}
\begin{algorithmic}[1]
\Require $x_0 \in \Rn$, $\lambda_0 \in \R^m$, $\epsilon > 0$, $r > 0$
\For{$k = 1, 2, \dots$}
  \State Solve $(QP_k)$ to obtain direction $d_k$
  \State $\alpha_k \gets \arg\min_{\alpha \ge 0}\, \Phi_r(x_k + \alpha d_k)$
  \State Update multipliers $\lambda_{k+1}$
...
\section{Convex Functions and Duality}
...
\subsection{Fenchel conjugate and duality}
\begin{theorem}
...
\section{Subdifferentials and Optimality Conditions}
\subsection{Subdifferentials of convex functions}
...
\section{Nonsmooth Differentials}

\subsection{Clarke derivative}
...
\subsection{Bouligand differentiability}
...
\subsection{Abs-normal optimization}
...
\subsection{Moreau envelope and resolvent}
...
\subsection{Steepest descent and subgradients}
...
\subsection{Subgradient algorithm}
\begin{algorithm}
\caption{Subgradient method}
\begin{algorithmic}[1]
\Require $f:\Rn \to \R$ convex, $\epsilon > 0$, initial point $x^0 \in \Rn$
\State Compute initial subgradient $s^0 \in \partial f(x^0)$
\For{$k = 0, 1, 2, \dots$}
  \If{$\|s^k\| \le \epsilon$}
    \State \Return $x^k$
  \EndIf
  \State $d^k \gets -s^k / \|s^k\|$
  \State Choose step size $\alpha_k > 0$
  \State $x^{k+1} \gets x^k + \alpha_k d^k$
  \State Compute $s^{k+1} \in \partial f(x^{k+1})$
\EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
\caption{Cutting-plane method (Kelley)}
\begin{algorithmic}[1]
\Require $f:\Rn \to \R$ convex and Lipschitz, $x^0 \in \Rn$, tolerance $\epsilon \ge 0$
\For{$k = 1, 2, \dots$}
  \State Compute subgradient $g^k \in \partial f(x^k)$
  \State Update minorant $f_k^{se}(x) \gets \max_{1 \le j \le k}\,\{f(x^j) + (g^j)^\top(x - x^j)\}$
  \If{$f(x^k) - f_k^{se}(x^k) \le \epsilon$}
    \State \Return $x^k$
  \EndIf
  \State $x^{k+1} \gets \arg\min_{x \in D}\, f_k^{se}(x)$
\EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
\Require $f:\Rn \to \R$ convex, $0 < \gamma_- \le \gamma_+$, tolerance $\epsilon > 0$, $\rho \in (0,1)$
\State Choose $\gamma_0 \in [\gamma_-, \gamma_+]$ and initial bundle $J_0 = \{0\}$
\For{$k = 1, 2, \dots$}
  \State Solve \eqref{OF:PSE2} via KKT conditions to obtain $(d^k, \lambda^k, \xi^k)$
  \State Compute aggregate subgradient $v^k \gets -d^k / \gamma_k$
  \State Compute linearisation error $\epsilon_k \gets \textstyle\sum_{j \in J_k} \lambda_j \alpha_j^k$
  \If{$\|v^k\| \le \epsilon$ \textbf{or} $\epsilon_k \le \epsilon$}
    \State \Return $x^k$
  \EndIf
  \If{$\mathrm{ared}(d^k) \ge \rho \cdot \mathrm{pred}(d^k)$}
    \State \textbf{Serious step:} $x^{k+1} \gets x^k + d^k$
  \Else
    \State \textbf{Null step:} $x^{k+1} \gets x^k$
  \EndIf
  \State Update $\gamma_{k+1} \in [\gamma_-, \gamma_+]$ and bundle $J_{k+1}$
\EndFor
\end{algorithmic}
\end{algorithm}
